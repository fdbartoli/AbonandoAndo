...
export class SaldoComponent implements OnInit {
  ...
  constructor(private servicePago: IngresoService, private serviceCobro: EgresoService) { }
  ...
  registrosPositivos(){
    let suma : number = 0;
    this.servicePago.getPagosCuil(this.cuil).subscribe(response =>{
  ...
  registrosNegativos(){
    let suma : number = 0;
    this.serviceCobro.getCobrosCuil(this.cuil).subscribe(response =>{
      this.dataSource = response;
      for (let i: number = 0; i < this.dataSource.length; i++) {
          suma = suma + this.dataSource[i].monto;
        }
